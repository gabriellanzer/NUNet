	...
	public static class NUServer
	{
			...
			ushort broadcastPort = 56552, int reservedBufferedPackets = NUUtilities.MaxBufferedPackets)
		{
			if (!started)
			{
				...
				try
				{
					...
					//Setup broadcaster
					broadcaster = new UdpClient(broadcastPort);
		...
		private static void EndBroadcastReceive(IAsyncResult asyncResult)
		{
			try
			{
				...
				if (broadcaster == null || broadcaster.Client == null) return;
				byte[] buffer = broadcaster.EndReceive(asyncResult, ref senderEP);
				...
				if (buffer.Length > 0)
				{
					try
					{
						...
						{
							Packet response = m_onBroadcastPacketReceived(new Packet(buffer));
							if (response.data.Length > 0)
							{
								if (broadcaster == null || broadcaster.Client == null) return;
								broadcaster.Send(response.data, response.data.Length,
									senderEP);
							}
						}
						else
						{
							Packet response = new Packet(port.ToString(),
								messageFlag: Packet.TypeFlag.DATA);
							if (broadcaster == null || broadcaster.Client == null) return;
							broadcaster.Send(response.data, response.data.Length,
								senderEP);
						}
					...
					{
						Debug.LogError("Error while r sponding broadcast message: " + ex.Message);
					}
		...
		public static void Shutdown()
		{
			if (started)
			{
				...
				lock (clientsLock)
				{
					Packet discPacket = new Packet("Shutdown...", GetConnectedClients(), Packet.TypeFlag.DCONNECT);
